    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-400 text-xs">Powered by Google Gemini • India-specific analysis</p>
        </div>
      </div>
        ...
        {fetched && insights.length > 0 && (
          ...
          >
            <p className="text-xs text-gray-400 uppercase tracking-widest font-medium">
              🎯 Personalized for You (India-Specific)
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {insights.slice(0, 3).map((insight, idx) => (
                <motion.div
                  ...
                  className={`rounded-xl p-5 border flex flex-col gap-3 ${insightColors[idx]}`}
                >
            ...
            {insights.length > 3 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {insights.slice(3, 5).map((insight, idx) => (
                  <motion.div
                    key={idx + 3}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: (idx + 3) * 0.15, duration: 0.5 }}
                    className={`rounded-xl p-5 border flex flex-col gap-3 ${insightColors[idx + 3]}`}
                  >
                    <div className="flex items-center gap-2">
                      <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-xl shadow-lg">
                        {insightIcons[idx + 3]}
            ...
            <div className="flex items-center justify-center gap-2 pt-4">
              ...
              <span className="text-xs text-gray-500">Generated by Gemini AI • India Standards (CEA/BEE/BIS)</span>
              <div className="h-px bg-white/10 flex-1" />
            </div>
          </motion.div>
        )}
